...
<div class="container mt-5">
    ...
    <form id="editPembelianForm">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="no_pembelian" value="{{ $pembelian->no_pembelian }}" required>
        </div>
        <div class="mb-3">
            <label for="tgl_pembelian" class="form-label">Tanggal Pembelian</label>
            <input type="date" class="form-control" id="tgl_pembelian" value="{{ $pembelian->tgl_pembelian }}" required>
        </div>
        ...
        <table class="table table-bordered table-striped">
            ...
            <tbody id="selectedBarangs">
                ...
                @if($hargaBeli->barang)
                <tr>
                    <th scope="row">{{ $hargaBeli->barang->id }}</th>
                    <td>{{ $hargaBeli->barang->nama_barang }}</td>
                    <td><input type="number" class="form-control" value="{{ $hargaBeli->harga_beli }}" min="0" step="0.01" placeholder="Harga Beli"></td>
...
document.addEventListener('DOMContentLoaded', function() {
    function attachRemoveEvent() {
        document.querySelectorAll('.remove-barang-btn').forEach(button => {
    ...
    document.getElementById('selectBarangBtn').addEventListener('click', function() {
        ...
        // Tutup modal setelah barang dipilih
    ...
    document.getElementById('savePembelianBtn').addEventListener('click', function() {
        ...
        axios.post('{{ route("pembelian.update", $pembelian->id) }}', {
            _token: '{{ csrf_token() }}',
